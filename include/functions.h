...
void func_803621CC(void);
void *func_80361C24(void* object, void* destructor, void* reg_mem);
...
// autogenerated by split.py v0.1 at 2020-12-27 17:32:37.874431
extern "C" {
    void PSVECAdd(const Vec *src1, const Vec *src2, Vec *dst);
    void PSVECNormalize(const Vec *src, Vec *dst);
    void PSVECCrossProduct(const Vec *src1, const Vec *src2, Vec *dst);
    void PSVECScale(const Vec *src1, Vec *dst, float scale);
    void PSVECSubtract(const Vec *src1, const Vec *src2, Vec *dst);
    s16 cM_atan2s(float, float);
    double PSVECSquareMag(const Vec *vec);
